...
pdf("Excision_Distance_boxplot.pdf", height=7, width=4)
par(mar=c(7,4,4,2))
...
high <- t(high)
data <- c(all[,1], high[,1])
mat <- matrix(data, ncol=2)
df <- data.frame(values = mat[1:426], vars = rep(c("all","high"), times = c(413,13)))
...
pvalue(1, 1230000, 2, 400000, 1250000, 9e-7)
dev.off()