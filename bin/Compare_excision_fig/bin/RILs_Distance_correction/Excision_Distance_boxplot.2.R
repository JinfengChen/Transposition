...
pdf("Excision_Distance_boxplot.2.pdf", height=7, width=4)
par(mar=c(7,4,4,2))
high <- read.table("Excision_distance.boxplot.high.txt")
all <- read.table("Excision_distance.boxplot.all.txt")
#all <- t(all)
#high <- t(high)
data <- c(all[,1], high[,1])
mat <- matrix(c(data, 0), ncol=2)
df <- data.frame(values = mat[1:433], vars = rep(c("all","high"), times = c(413,20)))
values = mat[1:433]
vars = rep(c("all","high"), times = c(413,20))
...
text(1, -240000, "HEG4 mPing", xpd=TRUE, srt=35)
text(2, -240000, "High Excision mPing", xpd=TRUE, srt=35)
axis(2,seq(0, 1200000, by=200000), line=0, labels=seq(0, 1200, by=200))
wilcox.test(all[,1], high[,1])
pvalue(1, 1230000, 2, 400000, 1250000, 7e-6)
dev.off()