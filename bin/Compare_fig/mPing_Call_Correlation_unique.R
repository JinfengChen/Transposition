pdf("mPing_Call_Correlation_unique.pdf")
relocate <- read.table("RIL275_RelocaTE.sofia.sorted.table", skip=1)
relocatei <- read.table("RIL275_RelocaTEi_unique.summary.table", skip=1)
...
##RelocaTEi depth, hom/het/som and unique

relocatei_sortbydepth <- relocatei[order(relocatei[,5]),]
relocatei_calls <- relocatei_sortbydepth[,5]
barx <- barplot(relocatei_calls, ylim=c(0, 60), border=FALSE, col=c("cornflowerblue"), xlab='RILs', ylab='Sequence Depth')
step <- (max(barx)+0.6-0.9)/275
pos <- c(seq(1, 275, by=23), 275)
pos_marks <- step*pos
#axis(1,pos_marks, tck=-0.04, line=0.2,labels=rep("",13))
...
relocatei_sortbydepth <- relocatei[order(relocatei[,5]),]
relocatei_calls <- rbind(relocatei_sortbydepth[,11], relocatei_sortbydepth[,12], relocatei_sortbydepth[,13])
barx <- barplot(relocatei_calls, ylim=c(0, 700), border=FALSE, col=c("cornflowerblue","darkseagreen","gray"), xlab='RILs', ylab='mPing insertions')
step <- (max(barx)+0.6-0.9)/275
pos <- c(seq(1, 275, by=23), 275)
pos_marks <- step*pos
#axis(1,pos_marks, tck=-0.04, line=0.2,labels=rep("",13))
...
#marks <- as.integer(relocatei_sortbydepth[,5][pos])
#text(pos_marks, rep(-150, 13),offset=2,labels=as.integer(marks),srt=0,xpd=TRUE)
legend(220, 700, c("Homozygous", "Heterozygous", "Somatic"),bty="n",border="NA",lty=c(0,0),cex=1,fill=c("cornflowerblue","darkseagreen","gray"))

#unique
relocatei_sortbydepth <- relocatei[order(relocatei[,5]),]
relocatei_calls <- rbind(relocatei_sortbydepth[,11]-relocatei_sortbydepth[,14], relocatei_sortbydepth[,14])
barx <- barplot(relocatei_calls, ylim=c(0, 700), border=FALSE, col=c("cornflowerblue","darkseagreen"), xlab='RILs', ylab='mPing insertions')
step <- (max(barx)+0.6-0.9)/275
pos <- c(seq(1, 275, by=23), 275)
pos_marks <- step*pos
#axis(1,pos_marks, tck=-0.04, line=0.2,labels=rep("",13))
...
#marks <- as.integer(relocatei_sortbydepth[,5][pos])
#text(pos_marks, rep(-150, 13),offset=2,labels=as.integer(marks),srt=0,xpd=TRUE)
legend(220, 700, c("Shared", "Unique"),bty="n",border="NA",lty=c(0,0),cex=1,fill=c("cornflowerblue","darkseagreen"))
