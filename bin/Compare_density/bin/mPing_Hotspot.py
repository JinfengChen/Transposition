...
def win_mping(fastaseq, win, gff):
    bed = 'MSU7.SplidingWin.bed'
    intersect = 'MSU7.SplidingWin.mPing.intersect'
    if not os.path.isfile(bed):
        ...
        ofile.close()
    if not os.path.isfile(intersect):
        intersect = 'MSU7.SplidingWin.mPing.intersect'
        cmd = '/opt/bedtools/2.17.0-25-g7b42b3b/bin/bedtools intersect -a %s -b %s -wo > %s' % (bed, gff, intersect) 
        os.system(cmd)
    
    data = defaultdict(lambda: int())
    with open (intersect, 'r') as filehd:
        for line in filehd:
            line = line.rstrip()
            if len(line) > 2: 
                unit = re.split(r'\t',line)
...
def main():
    ...
    mping = win_mping(fastaseq, win, args.gff)
    '''
