...
x   <- read.table('RIL230.RIL_mPing_Ping_Excision.table.txt', header=TRUE)
x_n <- read.table('RIL230.RIL_mPing_Ping_Excision.narrow_range.table.txt', header=TRUE)

print("Parental mPing vs Excision, RIL230")
cor.test(x[,2], x[,11])
print("Ping vs Excision, RIL230")
cor.test(x[,9], x[,11])

print("Parental mPing vs Excision, RIL230_narrow_range")
cor.test(x_n[,2], x_n[,11])
print("Ping vs Excision, RIL230_narrow_range")
cor.test(x_n[,9], x_n[,11])

par(mfrow=c(2,2))
plot_correlation_merged_parental_mping(x[,2], x[,11], '230 RILs')
plot_correlation_merged(x[,9], x[,11], '230 RILs')
plot_correlation_merged_parental_mping(x_n[,2], x_n[,11], '104 narrow range mPing RILs')
plot_correlation_merged(x_n[,9], x_n[,11], '104 narrow range mPing RILs')

par(mfrow=c(1,1))
plot_correlation_merged_parental_mping(x[,2], x[,11], 'Parental mPing Copy Number', 'Excision Number', '230 RILs')
plot_correlation_merged(x[,9], x[,11], 'Ping Copy Number', 'Excision Number', '230 RILs')
plot_correlation_merged_parental_mping(x_n[,2], x_n[,11], 'Parental mPing Copy Number', 'Excision Number', '104 narrow range mPing RILs')
plot_correlation_merged(x_n[,9], x_n[,11], 'Ping Copy Number', 'Excision Number', '104 narrow range mPing RILs')
