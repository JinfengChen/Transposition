...
pdf("ping_number.ALL.pdf")
data =read.table("RIL275_RelocaTEi.CombinedGFF.ALL.ping_number.summary", header = T)
expr = data[,2]
std = data[,3]
barx <- barplot(expr, col=c("black"), ylim=c(0,160), border=F, axis.lty=1, xlab='', ylab='')
...
mtext("mPing", side=2,font=3, at=ypos+11*1.3,line=3, cex=1, col="black")
mtext("number", side=2,font=1, at=ypos+25*1.3,line=3, cex=1, col="black")
dev.off()
