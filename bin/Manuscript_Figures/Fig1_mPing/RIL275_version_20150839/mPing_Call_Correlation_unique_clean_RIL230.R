...
barx <- barplot(temp_calls, ylim=c(0, 800), border=FALSE, col=c("cadetblue","chocolate"), xlab='Insertion Size (bp)', ylab='mPing insertions', main="TEMP")
step <- (max(barx)+0.6-0.9)/230
pos <- c(seq(1, 230, by=23), 230)
pos_marks <- step*pos
axis(1,pos_marks, tck=-0.04, line=0.2,labels=rep("",11))
...
barx <- barplot(relocatei_calls, ylim=c(0, 1200), border=FALSE, col=c("cornflowerblue","darkseagreen","gray"), xlab='Insertion Size (bp)', ylab='mPing insertions', main="RelocaTE2")
step <- (max(barx)+0.6-0.9)/230
pos <- c(seq(1, 230, by=23), 230)
pos_marks <- step*pos
axis(1,pos_marks, tck=-0.04, line=0.2,labels=rep("",11))
axis(1,c(0.9,pos_marks[length(pos)]), line=0.2,labels=c("",""))
marks <- as.integer(relocatei_sortbysize[,2][pos])
text(pos_marks, rep(-150, 11),offset=2,labels=as.integer(marks),srt=0,xpd=TRUE)
...
barx <- barplot(relocate1_calls, ylim=c(0, 1200), border=FALSE, col=c("cornflowerblue","darkseagreen","gray"), xlab='Insertion Size (bp)', ylab='mPing insertions', main="RelocaTE")
step <- (max(barx)+0.6-0.9)/230
pos <- c(seq(1, 230, by=23), 230)
pos_marks <- step*pos
axis(1,pos_marks, tck=-0.04, line=0.2,labels=rep("",11))
...
axis(1,c(0.9,pos_marks[length(pos)]), line=0.2,labels=c("1","230"))
#marks <- as.integer(relocatei_sortbydepth[,5][pos])
