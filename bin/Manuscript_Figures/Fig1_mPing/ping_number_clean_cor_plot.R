plot_correlation_merged <- function(x, y, xlab, ylab, title, ...){
    plot(x, y, xlab= '', ylab= '', main= title, cex.main=1)
    #regression line
    ...
    mtext("mPing", side=2,font=3, at=ypos+64,line=3, cex=0.8, col="black")
    mtext("number", side=2,font=1, at=ypos+100,line=3, cex=0.8, col="black") 
}
...
x_n <- read.table("RIL230_RelocaTEi.CombinedGFF.characterized.clean.mping.shared_unique_table.ping_code.narrow_range.txt", header=T)
x_h <- read.table("RIL230_RelocaTEi.CombinedGFF.characterized.clean.mping.shared_unique_table.ping_code.high_depth.txt", header=T)
x_hn <- read.table("RIL230_RelocaTEi.CombinedGFF.characterized.clean.mping.shared_unique_table.ping_code.high_narrow.txt", header=T)
hist(x[,2], xlab = 'HEG4 mPing copy number', main='')
plot(density(x[,2]), xlim=c(40, 300), xpd=TRUE, xlab = 'HEG4 mPing copy number', main='')
abline(v=c(163, 203), col=c('blue', 'blue'))
hom <- x[,6]
hom_n <- x_n[,6] 
hom_h <- x_h[,6]
hom_hn <- x_hn[,6]
...
plot_correlation_merged(ping_n, hom_n, 'Ping Copy Number', 'Unique homozygous mPing number', 'Parental mPing range (163-203)')
...
plot_correlation(ping_n, hom_n, 'Ping Copy Number', 'Unique homozygous mPing number', 'Parental mPing range (163-203)')
