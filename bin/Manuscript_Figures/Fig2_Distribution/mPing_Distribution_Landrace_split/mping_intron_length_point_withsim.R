...
pdf("simulate_intron_length_point_withsim.pdf")

par(mar=c(6,4,4,2), cex=1.2)
sim <- read.table("../mPing_distr/results_simulationV2_TSD9mer_somatic/Simulate.TSD9mer.SomaticMat.intron.length.distr")
som <- read.table("../mPing_distr/HEG4.intron.length.distr")
EG4 <- read.table("../mPing_distr/EG4.intron.length.distr")
str <- read.table("../mPing_distr/A119.intron.length.distr")
ril <- read.table("../mPing_distr/A123.intron.length.distr")
...
legend('topright', bty='n', border='NA', lty= c(1,2,3,4,5), pch = c(1,4,2,3,20), cex=1 , lwd = 2 ,col=c("aquamarine3", "darkgoldenrod4", "steelblue2", "sandybrown", "dim gray"), c("HEG4", "EG4", "A123", "A119","Simulation"))
mtext("Length of Intron (bp)", side=1,cex=1.2, at=4.5,line=4)
