...
def subsample(infile, cycle, size):
    data = []
    with open (infile, 'r') as filehd:
        for line in filehd:
            line = line.rstrip()
    ...
    for i in range(cycle):
        for j in range(size):
            ...
            print mping, data[idx]
            sample[mping] = int(data[idx])
    return sample
...
def main():
    ...
    sample = subsample('HEG4.mRNA.5primer.distance.list', 100, 20) 
    intron_len_sum(sample, 2000, 'HEG4.mRNA.5primer.distance.subsample.distr') 
