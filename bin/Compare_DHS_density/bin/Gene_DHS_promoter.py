    ...
    with open (infile, 'r') as filehd:
        for line in filehd:
            ...
            if len(line) > 2: 
                ...
                if unit[6] == '+':
                    if int(unit[12]) < int(unit[3]) and int(unit[12]) > int(unit[3]) - 1000:
                        region[gene] += int(unit[13]) - int(unit[12]) + 1
                    elif int(unit[13]) < int(unit[3]) and int(unit[13]) > int(unit[3]) - 1000:
                        region[gene] += int(unit[13]) - int(unit[12]) + 1
                elif unit[6] == '-':
                    if int(unit[12]) < int(unit[4]) + 1000 and int(unit[12]) > int(unit[4]):
                        region[gene] += int(unit[13]) - int(unit[12]) + 1
                    elif int(unit[13]) < int(unit[4]) + 1000 and int(unit[13]) > int(unit[4]):
                        region[gene] += int(unit[13]) - int(unit[12]) + 1
